...
3. **`no-ai-tells` gates it last** — if that skill is installed, it runs on the final prose and wins on conflict, with one exemption:

**The VOC exemption:** verbatim customer quotes are exempt from the no-ai-tells gate and from voice smoothing. Customers talk in fragments, clichés, and broken grammar; that is the point. The exemption covers only true verbatim material (quoted or headline-lifted), not copy you wrote *around* it. If a gate rule and a verbatim quote collide, the quote stays and the surrounding prose adapts.
